...
import (
	"github.com/dataStrcut-tool/internal/errs"
	"github.com/dataStrcut-tool/internal/slice"
)
...
// Append 往ArrayList中追加元素
func (a *ArrayList[T]) Append(ts ...T) error {
	a.vals = append(a.vals, ts...)
	return nil
}

// Add 在ArrayList中下标为index的位置插入一个元素
// 当index等于ArrayList的长度时候，相当于Append
...
// - 如果容量 > 2048，并且长度小于容量一半，那么就会缩容为原来的 5/8
// - 如果容量 (64, 2048], 如果长度是容量的 1/4，那么就会缩容为原本的一半
// - 如果此时容量 <= 64,那么我们将不会执行缩容。在容量很小的情况下，浪费的内存很少，没必要消耗 CPU去执行缩容
