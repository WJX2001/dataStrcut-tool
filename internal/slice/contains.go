...
// ContainsFunc 判断 src 里面是否存在list
// 优先调用Contains
...
// ContainsAny 判断 scr 里面是否存在 dst 中的任何一个元素（有一个就返回true）
...
// ContainsAll 判断 src 里面是否存在 dst 中的所有元素（全部存在才返回true）
